...
class SleepQualityByMovementsPerHour extends React.Component {
  constructor(props) {
    super(props);
    let date = new Date();
  ...
  componentDidMount() {
    ...
    SleepSessionService.index(this.state.startDate, this.state.endDate).then(
      response => {
        let data = response.map(sleepSession => ({ x: sleepSession.movements_per_hour, y: sleepSession.sleep_quality}));
        this.setState({sleepQualityByMovementsPerHour: data})
      },
      error => {
        console.log(error);
      });
    ...
    return(
      <Card>
        <Card.Header className="text-center" as="h4">
          Sleep Quality by Movements per hour
        </Card.Header>
        <Card.Body>
          {this.formComponent()}
          {!this.state.sleepQualityByMovementsPerHour.length ? this.noGraphDate() : this.graphComponent()}
        </Card.Body>
      </Card>
    );
  };
  ...
  graphComponent = () => {
    ...
    return (
      ...
      >
        <VictoryGroup
          data={this.state.sleepQualityByMovementsPerHour}
...
export default SleepQualityByMovementsPerHour;
