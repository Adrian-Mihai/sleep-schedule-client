...
class SleepQualityByTimeInBed extends React.Component {
  constructor(props) {
    super(props);
    let date = new Date();
  ...
  componentDidMount() {
    ...
    SleepSessionService.index(this.state.startDate, this.state.endDate).then(
      response => {
      let data = response.map(sleepSession => ({ x: sleepSession.time_in_bed, y: sleepSession.sleep_quality}));
      this.setState({sleepQualityByTimeInBed: data})
    },
      error => {
        console.log(error);
      });
 ...
 componentWillUnmount() {
    ...
    return(
      <Card>
        <Card.Header className="text-center" as="h4">
          Sleep Quality by Time in bed
        </Card.Header>
        <Card.Body>
          {this.formComponent()}
          {!this.state.sleepQualityByTimeInBed.length ? this.noGraphDate() : this.graphComponent()}
        </Card.Body>
      </Card>
    );
  };
  ...
  graphComponent = () => {
    ...
    >
      <VictoryGroup
        data={this.state.sleepQualityByTimeInBed}
...
export default SleepQualityByTimeInBed;
